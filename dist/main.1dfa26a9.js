...
var formAdd = document.querySelector('[data-form]');
var formInput = document.querySelector('[data-form-input]');
var template = document.querySelector('[data-todo]');
...
var checkCount = todoList.filter(function (todo) {
...
function firstCheckable() {
  ...
  var listItem = list.querySelectorAll('[data-todo-item]');
  var itemOrdersArr = Array.from(listItem).map(function (item) {
    return Number(item.dataset.order);
  });
  var itemFirstOrder = Math.min.apply(Math, _toConsumableArray(itemOrdersArr));
  var itemFirst = Array.from(listItem).find(function (item) {
    return Number(item.dataset.order) == itemFirstOrder;
  });
  var todoFirst = todoList.find(function (todo) {
    return todo.id === itemFirst.dataset.id;
  });
...
function checkable() {
  ...
  todoList.forEach(function (todo) {
    ...
    var next = Number(todo.order);
    next++;
    var nextTodo = todoList.find(function (todo) {
      return Number(todo.order) === next;
    });
    nextTodo.checkable = todo.check;
    var todoCheck = todo.check;

    if (!todoCheck) {
      var nextOrder = Number(todo.order);
      todoList.forEach(function (todo) {
  ...
  listItem.forEach(function (item) {
    var actualTodo = todoList.find(function (todo) {
      return todo.id === item.dataset.id;
    });
...
function showTodo(todo) {
  var showTodo = template.content.cloneNode(true);
  var todoItem = showTodo.querySelector('[data-todo-item]');
  todoItem.setAttribute('data-id', todo.id);
  todoItem.setAttribute('data-order', todo.order);
  todoItem.setAttribute('data-checkable', todo.checkable);
  todoItem.setAttribute('draggable', true);
  todoItem.style.order = "".concat(todo.order);
  var todoOrder = showTodo.querySelector('[data-todo-order]');
  todoOrder.innerHTML = todo.order;
  var todoCheck = showTodo.querySelector('[data-todo-checkbox]');
  todoCheck.checked = todo.check;
  todoItem.setAttribute('data-check', todoCheck.checked);
  var todoSpan = showTodo.querySelector('[data-todo-span]');
  todoSpan.innerHTML = todo.name;
  list.appendChild(showTodo);
}

function defineOrder() {
  todoList.forEach(function (todo, pos) {
    todo.order = pos + 1;
  });
}

function updateCheckCount() {
  checkCount = todoList.filter(function (todo) {
    return todo.check === true;
  });
  listCount[0].innerHTML = checkCount.length;
}
...
function updateOrder() {
  ...
  todoOrder.forEach(function (todo, pos) {
    todo.dataset.order = todoList[pos].order;
    todo.style.order = "".concat(todo.dataset.order);
